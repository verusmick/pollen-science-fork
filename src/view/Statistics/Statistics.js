...
export default ({
  ...
  created() {
    useMeta({
      title: "Pollen Science - Statistik",
      meta: [
  ...
  data: () => {
    const preSelectedPollen = ['Corylus', 'Alnus', 'Fraxinus', 'Betula', 'Poaceae', 'Artemisia', 'Ambrosia']
  ...
  methods: {
    ...
    refreshStats: function () {
      if (!!this.selectedLocation && !!this.selectedPollen) {
        new SeasonsStore().getStatistics(this.selectedLocation.id, this.selectedPollen).then((stats) => this.stats = stats.statistics)
      }
    },
